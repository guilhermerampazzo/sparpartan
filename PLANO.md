...
### M14 — Orçamentos (desconto, PDF compacto, voltar correto), Taxas (vínculo automático) e Central de Pendências (painel operacional)
Itens 4–6 do pedido, testados E2E:
- **Orçamentos**: form abre com **1 item visível** (sem clique em "Adicionar itens") com colunas **Produto/Serviço, Qtd., Valor unitário, Desconto, Total** (coluna `desconto` em `orcamento_itens`, migration 0037 — desconto em R$ por item, total geral com descontos). **Voltar correto**: "Gerenciar contas" navega com `?origem=/orcamentos/novo` e o BackButton de Contas Bancárias (lista e edição) usa a origem como fallback — quem veio do orçamento volta para ele, não para Configurações. **PDF compacto**: margens 48→22px, espaçamentos reduzidos (~50%) — orçamento pequeno cabe em 1 página (verificado: `/Count 1`); tabela ganhou coluna "Desc." (desconto).
- **Taxas**: o **anexo do boleto dispara o reconhecimento automaticamente** (antes eram 2 passos) — preenche descrição/valor/vencimento/número, **acha o cliente por CPF ou nome e vincula o processo ativo dele** (selects preenchidos, filtro de processos por cliente). Detecção de pagamento: o extrator procura indícios ("PAGAMENTO REALIZADO", "COMPROVANTE", "QUITADA") e marca o checkbox "documento indica pagamento" — `criarTaxa` salva como `pago` quando confirmado. **Interface única de PDF**: só "Abrir" (inline) na listagem. **Reflexo no processo**: `marcarTaxaComoPaga` (e taxa criada paga) roda `reclassificarProcesso` no processo vinculado + auditoria.
- **Central de Pendências**: cada pendência virou **cartão** com motivo, badges, **cliente clicável, processo clicável** (nome do serviço via join), prazo com cor de urgência, responsável e observações. Botão **"Dar andamento"** abre o módulo certo: processo → `/processos/[id]`, só cliente → `/clientes/[id]`, senão edição da pendência. Histórico separado em **Concluídas (Reabrir)** e **Arquivadas (Restaurar)** — nova action `restaurarPendencia` evita perda acidental de arquivamento (auditoria).

### M15 — Datas BR, Fluxo padrão dos processos, Processos como serviço, Pendências unificadas, Documentos por serviço e Arquivos-pasta (itens 13–18)
Testado E2E no navegador:
- **Datas (13)**: helper `lib/datas.ts` (`formatarDataBR`, `dataIsoParaBR`); corrigida a exibição crua de vencimentos (era "2026-08-11" → "11/08/2026"); demais módulos já usavam `toLocaleDateString("pt-BR")`. ISO interno (banco/inputs) inalterado.
- **Processos (14)**: listagem com `BackButton` e sem "+ Novo Atendimento" (criação de processo pelo Novo Cliente); **Processos sai do menu principal e entra em Clientes** (ao lado de Lixeira/Indicações/Aniversariantes); detalhe do cliente sem "+ Novo Processo" com empty explicando o fluxo.
- **Fluxo padrão (15)**: novo enum `aberto → processo_preenchido → processo_assinado → aguardando_pagamento → protocolado → concluido` (migration 0038; UPDATE migrou `documentos_pendentes→processo_preenchido`, `pronto_para_protocolo→processo_assinado`, `aguardando_retorno_marinha→protocolado`; antigos seguem no enum como leitura). Coluna nova **`exigencia_observacao`** — ao protocolar, o operador informa a exigência (ex.: "apresentar documento X"), exibida no processo (card de protocolo) e no cliente (card do processo). `reclassificarProcesso` não regride etapas manuais. Reflexos automáticos via `status.ts`: Home, Central de Pendências, Pipeline/Central Operacional, abas de `/processos` (nova aba Pagamento). `marcarAguardandoRetornoMarinha` removida.
- **Pendências (16)**: `/pendentes` removido do menu; rota virou **redirect → `/pendencias`** (Central é o único lugar de controle).
- **Documentos (17)**: página reorganizada **por serviço** — Arrais Amador, Motonauta, Embarcação Esporte e Recreio, Embarcação Comercial, Obras Náuticas, Outros (este pega categorias ativas não cobertas). **Submenus da sidebar removidos**; botão "+ Gerar Documento" da página inicial removido (geração fica dentro de cada área); slugs antigos (`escola-nautica` etc.) redirecionam para `/documentos`; `BackButton` em todas as áreas.
- **Arquivos (18)**: pasta digital **colapsada por cliente** ("JOÃO DA SILVA ▸" + contagem; expande com tudo do cliente: arquivos, boletos, fotos, comprovantes, documentos gerados + upload). **Histórico por arquivo** (migration 0038): `arquivo_alteracoes` + colunas `criado_por_id/atualizado_por_id/atualizado_em` em `arquivos`; actions registram quem criou/alterou; UI mostra "Arquivo enviado por Administrador — 13/08/2026, 11:45" (expansível).

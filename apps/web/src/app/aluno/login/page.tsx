...
import { signInAluno } from "@/lib/auth-aluno";
...
async function login(formData: FormData) {
  ...
  const callbackUrl = (formData.get("callbackUrl") as string) || "/aluno";

  try {
    await signInAluno("aluno", { email, senha, redirectTo: callbackUrl });
  } catch (error) {
    if (error instanceof AuthError) {
      redirect(`/aluno/login?erro=1&callbackUrl=${encodeURIComponent(callbackUrl)}`);
    }
    throw error;
  }
}
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm rounded-xl border border-outline-variant bg-surface-container-lowest p-8 shadow-md">
        ...
        <p className="mb-6 text-sm text-on-surface-variant">Área do Aluno</p>
        ...
        <form action={login} className="space-y-4">
          <input type="hidden" name="callbackUrl" value={params.callbackUrl ?? "/aluno"} />
          <div>
